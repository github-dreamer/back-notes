...
![img](image/relationship-with-mybatis.png)
...
<img src="image/mybatis-plus-framework.jpg" alt="framework" style="zoom:50%;" />
...
- **使用`Spring Initializer`快速初始化一个 Spring Boot 工程**
  ...
  <img src="image/image-20220519140839640.png" alt="image-20220519140839640" style="zoom:80%;" />

  <img src="image/image-20220519141335981.png" alt="image-20220519141335981" style="zoom:80%;" />

  <img src="image/image-20220519141737405.png" alt="image-20220519141737405" style="zoom:80%;" />

  <img src="image/image-20220519141849937.png" alt="image-20220519141849937" style="zoom:80%;" />
...
- **安装`Lombok`插件**(较新的idea已经内置lombok)

  <img src="image/image-20220519143257305.png" alt="image-20220519143257305" style="zoom:80%;" />
...
- **控制台打印查询结果**
  ...
  ![image-20220519150454211](image/image-20220519150454211.png)
...
- 我们将表`user`更名为`t_user`，测试查询功能
  ...
  ![image-20220520093844842](image/image-20220520093844842.png)

- 程序抛出异常，**Table 'mybatis_plus.user' doesn't exist**，因为现在的表名为`t_user`，而默认操作的表名和实体类型的类名一致，即`user`表

  ![image-20220520094126411](image/image-20220520094126411.png)
...
- 我们实体类中的属性`id`改为`uid`，将表中的字段`id`也改为`uid`，测试添加功能
  ...
  ![image-20220520100939157](image/image-20220520100939157.png)

  ![image-20220520100715109](image/image-20220520100715109.png)

- 程序抛出异常，**Field 'uid' doesn't have a default value**，说明MyBatis-Plus没有将`uid`作为主键赋值

  ![image-20220520101317761](image/image-20220520101317761.png)
...
```
备注:  关于@TableId使用的三种情况:
		1、实体类中主键为id,数据库主键也为id,此时可以不用声明@TableId,因为mybatis-plus默认会使用id作为主键
		2、当实体类主键和数据库主键名一致,但不是id,需要声明@TableId即可
		3、当实体类主键和数据库主键名不一致时,需要声明@TableId("数据库主键名"),此时注解的值需要写上数据库主键的名
```
...
![image-20220520103030977](image/image-20220520103030977.png)
...
- **数据库中创建逻辑删除状态列，设置默认值为0**
  ...
  ![image-20220520134529809](image/image-20220520134529809.png)

- **实体类中添加逻辑删除属性**

  ![image-20220520134636112](image/image-20220520134636112.png)
...
- **测试删除功能，真正执行的是修改**
  ...
  ![image-20220520135637388](image/image-20220520135637388.png)

- **此时执行查询方法，查询的结果为自动添加条件`is_deleted=0`**

  ![image-20220520140036445](image/image-20220520140036445.png)
...
![image-20220521092812125](image/image-20220521092812125.png)
...
```
备注: 1、QueryWrapper和UpdateWrapper中的字段都是写死的,而LambdaQueryWrapper和LambdaUpdateWrapper是动态的选择列名,因此比较推荐后两者
	 2、利用Wrapper可以方便地实现对数据的 查、删、改
```
...
- **添加配置类`MyBatisPlusConfig`**
  ...
  public class MyBatisPlusConfig {
      // MybatisPlusInterceptor中配置插件,springboot笔记中也有
      @Bean
...
```
备注: 乐观锁 => 加一个版本号, 悲观锁 => 常见的方式(比如将方法改为同步方法)
```
...
- **执行结果**
  ...
  ![image-20220521225803162](image/image-20220521225803162.png)
...
- **优化执行流程**
  ...
  ![image-20220521230448577](image/image-20220521230448577.png)
...
- **数据库表添加字段`sex`**
  ...
  ![image-20220521231317777](image/image-20220521231317777.png)
...
![image-20220522113049945](image/image-20220522113049945.png)
...
![image-20220522115718361](image/image-20220522115718361.png)
...
- 在IDEA中与数据库建立链接
  ...
  ![image-20220522120758740](image/image-20220522120758740.png)

- 填写数据库信息并保存

  ![image-20220522121434468](image/image-20220522121434468.png)

- 找到我们需要生成的表点击右键

  ![image-20220522121613909](image/image-20220522121613909.png)

- 填写完信息以后下一步

  ![image-20220522122127649](image/image-20220522122127649.png)

- 继续填写信息

  ![image-20220522122525598](image/image-20220522122525598.png)

- **大功告成（真特么好用yyds）**

  ![image-20220522122612334](image/image-20220522122612334.png)
...
![image-20220522123143852](image/image-20220522123143852.png)

![image-20220522123202310](image/image-20220522123202310.png)
